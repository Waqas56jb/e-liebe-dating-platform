...
import { getMyProfile, getStats } from '../../services/api';

const FALLBACK = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=400&q=80';

export default function MyProfileScreen({ language = 'de', reloadKey = 0, onEdit, onPreferences, onPrivateMode, onPrivacy, onSettings, onPremium }) {
  const t = makeT(language);
  const [profile, setProfile] = useState(null);
  const [stats, setStats] = useState({ likes_received: 0, matches_count: 0, profile_views: 0 });
  ...
  const photo = profile?.photos?.[0] || FALLBACK;
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: spacing.xxl }}>
            <View style={styles.hero}>
              <View style={styles.avatarRing}>
                ...
                <Pressable style={styles.editAvatar} onPress={onEdit}><Ionicons name="camera" size={16} color={colors.white} /></Pressable>
              </View>
              <Text style={styles.name}>{profile?.name || ''}{profile?.age ? `, ${profile.age}` : ''}</Text>
              {profile?.city ? (
                <Text style={styles.city}><Ionicons name="location-outline" size={13} color="rgba(255,255,255,0.7)" /> {profile.city}</Text>
              ) : null}

              <View style={styles.completion}>
                <View style={styles.completionBarBg}><View style={[styles.completionBar, { width: `${completion}%` }]} /></View>
                <Text style={styles.completionText}>{completion}% {pick(P.complete, language)}</Text>
              </View>
            ...
            <View style={styles.stats}>
              <Stat value={stats.likes_received} label={pick(P.likes, language)} />
              <View style={styles.statDivider} />
              <Stat value={stats.matches_count} label={pick(P.matches, language)} />
              <View style={styles.statDivider} />
              <Stat value={stats.profile_views} label={pick(P.views, language)} />
            </View>
...
function computeCompletion(p) {
  if (!p) return 0;
  let total = 6, done = 0;
  if (p.photos?.length) done++;
  if (p.name) done++;
  if (p.age) done++;
  if (p.bio) done++;
  if (p.interests?.length) done++;
  if (p.goal) done++;
...
const styles = StyleSheet.create({
  ...
  editAvatar: { position: 'absolute', bottom: 4, right: 4, width: 34, height: 34, borderRadius: 17, backgroundColor: colors.rose, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: colors.ink },
  ...
  editBtn: { flexDirection: 'row', alignItems: 'center', marginTop: spacing.lg, backgroundColor: colors.glass, borderWidth: 1, borderColor: colors.glassBorder, paddingHorizontal: spacing.xl, paddingVertical: 12, borderRadius: 999 },
  editText: { ...typography.button, color: colors.white, marginLeft: 8 },
  stats: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', marginHorizontal: spacing.xl, marginTop: spacing.xl, backgroundColor: colors.glass, borderWidth: 1, borderColor: colors.glassBorder, borderRadius: radius.lg, paddingVertical: spacing.lg },
